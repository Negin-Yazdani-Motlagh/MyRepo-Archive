...
def load_dict91_skills():
    ...
    excel_path = os.path.join(os.path.dirname(script_dir), 
                             "Dictionary_9.1 AILLM", "Dictionary of soft skills (9.1).xlsx")
...
def load_dict11_skills():
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    excel_path = os.path.join(os.path.dirname(script_dir), 
                             "Dictionary_11_AILLM", "Dictionary of soft skills (11).xlsx")
...
def analyze_job_posts():
    ...
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "combined_categories_AI_LLM.json")
    ...
    plt.plot(years_list, dict91_normalized, 
            ...
            label='Dictionary 9.1 AILLM (Communication + Collaboration + Customer Service)', 
            color=colors[0], 
    ...
    plt.plot(years_list, dict11_normalized, 
            marker='o',  # Add markers for exact data points
            label='Dictionary 11 AILLM (Interpersonal & Leadership Skills)', 
            color=colors[1], 
            linestyle=line_styles[1],
            linewidth=2, 
            markersize=8)
    
    plt.title('Comparison of Combined Categories (AI_LLM)\n(Exact Occurrences / Total Posts for Each Year)', 
              fontsize=14, pad=20)
    ...
    # Save the plot
    plot_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "combined_categories_AI_LLM_comparison.png")
    ...
    for year, dict91_val, dict11_val in zip(years_list, dict91_normalized, dict11_normalized):
        print(f"\nYear {year}:")
        print(f"Dictionary 9.1 AILLM Combined: {dict91_val:.3f}")
        print(f"Dictionary 11 AILLM Interpersonal: {dict11_val:.3f}")
