...
def load_skills_dictionary():
    ...
    for _, row in df.iterrows():
        ...
        # Skip entries containing 'responsible'
        if subcategory and 'responsible' not in subcategory:
...
def analyze_job_posts():
    ...
    for year in years:
        ...
        results[year] = {
            ...
            'categories': defaultdict(int)  # Changed to store only category totals
        }
    ...
    for year in years_list:
        ...
        print("Normalized headcategory occurrences (occurrences/total posts):")
        for category, norm_value in sorted_categories:
            print(f"{norm_value:.3f} - {category}")
    
    # Create the plot
    plt.figure(figsize=(15, 8))
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c']  # Set distinct colors
    markers = ['o', 's', '^']  # Set distinct markers
    
    for category, color, marker in zip(categories, colors, markers):
        plt.plot(years_list, plot_data[category], marker=marker, label=category, 
                color=color, linewidth=2, markersize=8)
    
    plt.title('Normalized Skill Category Trends Over Time\n(Occurrences / Total Posts)', 
              fontsize=14, pad=20)
